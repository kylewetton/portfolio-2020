...
const routes = ['home', 'projects', 'about-me'];

app.get('/invoke/:url', function(req, res) {
  const {url} = req.params;
  const page = routes.includes(url) ? url : '404';

  console.log(url, page);

  fs.readFile(`${__dirname}/views/${page}.html`, 'utf8', function (err, html) {
    res.send(html);
  });
});
